...
export class RabbitmqQueueModule {
  static async createWorkerMicroserviceOptions(
  ...
  static async createRecoveryMicroserviceOptions(
    options: RabbitmqQueueModuleOptions,
  ): Promise<ClientProviderOptions> {
    const config = RabbitmqQueueConfigFactory(options);
    const { deadLetter } = config;
  ...
  static register(options: RabbitmqQueueModuleOptions): DynamicModule {
    ...
    return {
      ...
      exports: [RabbitmqProducerClient],
      providers: [RabbitmqProducerClient],
    };
  }
}
